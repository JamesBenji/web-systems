...
$rec_del_q = "SELECT * FROM (SELECT * FROM tms_delivery INNER JOIN tms_order USING (order_id)) AS derived_table WHERE emp_id = '$emp_id' and delivery_status = 'completed'";
$rec_del_res = $connection->query($rec_del_q);

$row_arr = array();

if ($rec_del_res) {
    while ($row = $rec_del_res->fetch_assoc()) {
